...
def load_data(data_url, ESM, target_index, input_features, add_months, norm_target, lead_time):
    ...
    if add_months:
        ...
        inputs['Jan']= (climind.loc[(ESM)].reset_index().drop(columns=['year']).loc[:,'month']==1).values.astype(int)
        inputs['Feb']= (climind.loc[(ESM)].reset_index().drop(columns=['year']).loc[:,'month']==2).values.astype(int)
        inputs['Mar']= (climind.loc[(ESM)].reset_index().drop(columns=['year']).loc[:,'month']==3).values.astype(int)
        inputs['Apr']= (climind.loc[(ESM)].reset_index().drop(columns=['year']).loc[:,'month']==4).values.astype(int)
        inputs['May']= (climind.loc[(ESM)].reset_index().drop(columns=['year']).loc[:,'month']==5).values.astype(int)
        inputs['Jun']= (climind.loc[(ESM)].reset_index().drop(columns=['year']).loc[:,'month']==6).values.astype(int)
        inputs['Jul']= (climind.loc[(ESM)].reset_index().drop(columns=['year']).loc[:,'month']==7).values.astype(int)
        inputs['Aug']= (climind.loc[(ESM)].reset_index().drop(columns=['year']).loc[:,'month']==8).values.astype(int)
        inputs['Sep']= (climind.loc[(ESM)].reset_index().drop(columns=['year']).loc[:,'month']==9).values.astype(int)
        inputs['Oct']= (climind.loc[(ESM)].reset_index().drop(columns=['year']).loc[:,'month']==10).values.astype(int)
        inputs['Nov']= (climind.loc[(ESM)].reset_index().drop(columns=['year']).loc[:,'month']==11).values.astype(int)
        inputs['Dec']= (climind.loc[(ESM)].reset_index().drop(columns=['year']).loc[:,'month']==12).values.astype(int)   
...
def scale_norm_inputs(scale_norm, train_input, val_input, test_input, input_features):
    ...
    if scale_norm == "scale_01":
        ...
        train_input_scaled[:,:,:len(input_features)] = (train_input_scaled[:,:,:len(input_features)] - train_min) / (train_max - train_min)
        val_input_scaled[:,:,:len(input_features)] = (val_input_scaled[:,:,:len(input_features)] - train_min) / (train_max - train_min)
        test_input_scaled[:,:,:len(input_features)] = (test_input_scaled[:,:,:len(input_features)] - train_min) / (train_max - train_min)
    ...
    if scale_norm == "scale_11":
        ...
        # Apply scaling to all features, except optionally added one-hot encoded months:
        train_input_scaled[:,:,:len(input_features)] = 2 * (train_input_scaled[:,:,:len(input_features)] - train_min) / (train_max - train_min) - 1
        val_input_scaled[:,:,:len(input_features)] = 2 * (val_input_scaled[:,:,:len(input_features)] - train_min) / (train_max - train_min) - 1 
        test_input_scaled[:,:,:len(input_features)] = 2 * (test_input_scaled[:,:,:len(input_features)] - train_min) / (train_max - train_min) - 1 
    ...
    if scale_norm == "norm":
        ...
        train_input_scaled[:,:,:len(input_features)] = (train_input_scaled[:,:,:len(input_features)] - train_mean) / train_std
        val_input_scaled[:,:,:len(input_features)] = (val_input_scaled[:,:,:len(input_features)] - train_mean) / train_std
        test_input_scaled[:,:,:len(input_features)] = (test_input_scaled[:,:,:len(input_features)] - train_mean) / train_std

    # else: Keep raw input features.
    
    # Return scaled inputs and statistics from training data:
    return train_input_scaled, val_input_scaled, test_input_scaled, train_mean, train_std, train_min, train_max
...
def prepare_inputs_and_target(
    ...
    (
        ...
        test_input,
        test_target
    ) = train_val_test_split(inputs_split=inputs_split,
